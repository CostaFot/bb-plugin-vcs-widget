...
import type { BranchRef, CompareRef, CompareResult, GitError, PatchResult, TagList, WorkingTreeDiff } from "../contracts";
...
import { capPatch, readFailure as diffReadFailure } from "./diff-text";
import { runGit } from "./git";
import type { ActionContext } from "./actions";

const readFailure = (result: Parameters<typeof diffReadFailure>[0]): GitError => diffReadFailure(result, "compare");
...
/**
 * The endpoint a compare walks from: a branch by its full ref (so a tag of
 * the same name cannot shadow it), or a revision from the log resolved to
 * its commit sha.
 */
async function verifyRef(context: ActionContext, ref: CompareRef): Promise<string | GitError> {
  if (ref.kind === "revision") {
    const resolved = await runGit(
      ["rev-parse", "--verify", "--quiet", "--end-of-options", `${ref.revision}^{commit}`],
      readOptions(context),
    );
    const sha = resolved.stdout.trim();
    if (resolved.code !== 0 || sha === "") {
      return { code: "ref_not_found", message: `'${ref.revision}' is not a commit in this repository.` };
    }
    return sha;
  }
...
export async function compare(context: ActionContext, input: { base: CompareRef; target: CompareRef }): Promise<CompareResult> {
...
async function patchFor(context: ActionContext, revisionArgs: string[], path: string): Promise<PatchResult> {
  ...
  // The branch panels do not offer expand-context; only the log's commit
  // patch reads both complete sides.
  return { ok: true, path, ...capPatch(result.stdout), contents: null };
}

export async function comparePatch(
  context: ActionContext,
  input: { base: CompareRef; target: CompareRef; path: string },
): Promise<PatchResult> {
  const base = await verifyRef(context, input.base);
  if (isError(base)) return { ok: false, error: base };
  const target = await verifyRef(context, input.target);
