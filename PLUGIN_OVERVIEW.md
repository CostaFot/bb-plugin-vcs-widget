...
- An **Example todos** page in the left sidebar that adds, completes, and
  removes todos.
- A `bb vcs-group` command that does the same from a terminal.
...
The bundled skill tells an agent to read the list with `bb vcs-group list`, add
one todo at a time with `bb vcs-group add`, and close finished work with
`bb vcs-group done`.
